...
public class UserObjectJobRunner {
  ...
  public int start(ConfigurableApplicationContext context, boolean launchedByDBA,
                   String objectFilter, String typeFilter, String typeFilterMode) throws Exception {
    try {
      ...
      logger.info(String.format("Will try to process schema %s %s ",
                                schemaList.size() > 1 ? "list" : "", schemaList));
      ...
      logger.info(String.format("Processing schema %s %s completed ",
                                schemaList.size() > 1 ? "list" : "", schemaList));
